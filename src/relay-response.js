...
export function normalizeRelayJobError(error, { knownWorkerError = false } = {}) {
  ...
  const exactOfferUnavailable =
    /(?:không trả đúng|trả nhiều record cho)\s+offer id/i.test(rawMessage)
    || /exact offer id.+(?:not found|unavailable)/i.test(rawMessage);
  const productUnavailable =
    PRODUCT_UNAVAILABLE_CODES.has(rawCode) || exactOfferUnavailable;
